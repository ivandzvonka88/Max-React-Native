...
import { FormLiftInfo, FormPersonalInfo, ProgramInfo } from '../../components/UserForms'
import { handleSave } from './api'
import { adjustMaxs } from './utilsPayment'
...
const handleChange = (text, field_name, parent) => {
  ...
  if (['bodyweight', 'height', 'promo_code'].includes(field_name)) {
    parent.setState({ [field_name]: text })
  }
...
class PaymentScreen extends React.Component {
  ...
  handleDropdown = (child) => {
    console.log('picked child', child)
    ...
    if (fieldName.includes('Styles')) {
      ...
      var changed = this.state.changed
      changed = changed.concat("frequency")

      this.setState({ show: false, changed, frequency: child.value })
    } else if (fieldName === "fitness") {
      var changed = this.state.changed
      changed = changed.concat("fitness")

      this.setState({ show: false, changed, fitness: child.value })
    }
  }

  showDropDown(fieldName, buttonStyle, placeholderText, choices, description) {
  ...
  purchaseButton = () => {
    var style = styles.purchaseButton
    return (
          ...
          <SafeAreaView style={styles.pageContainer}>
            ...
            <View style={styles.accordionWrapper}>
              <Accordion
                ...
                key="program_info"
              >
                < ProgramInfo parent={this} handleChange={handleChange} />
              </Accordion>
            </View>
            <View style={styles.formRow} >
              <AppTextInput
                ...
                autoCorrect={false}
                onChangeText={(text) => handleChange(text, 'promo_code', this)}
              />
            </View>
            ...
            <DropDownMenu
              ...
              description={this.state.dropdownPlaceholderText === 'fitness'}
            />
